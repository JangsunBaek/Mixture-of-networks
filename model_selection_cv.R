...
#      for (t in 1:n_Lambda) {
#        y_CLC<-clnmm(x, g=s, n_level, lambda=Lambda[t], initmax, itmax, tol_logL, init_para = NULL)
#        cCLC[s-min_G+1,t] <- 0
#        EN <- 0
#        cCLC[s-min_G+1,t]<-loglike_pred.clnmm(n_level, obs_freq, y_CLC$pivec, y_CLC$theta_marg, y_CLC$theta_cond)
#        for (j in 1:(p-1)) {
#          for (k in (j+1):p) {
#            Xjk <- x[,c(j,k)]
#            tau <- tau.clnmm(Xjk, j, k, g=s, y_CLC$pivec, y_CLC$theta_marg, y_CLC$theta_cond) 
#            taulogtau <- apply(tau, 1, function(tau) {tau*log(tau)})
#            EN <- EN + sum(taulogtau)
#          }
#        }
#        cCLC[s-min_G+1,t] <- cCLC[s-min_G+1,t] - EN
#cat("g=",s," ", "lambda=",Lambda[t]," ", "cCLC=",cCLC[s-min_G+1,t],"\n")          
#      }
#    }    
...
# Transform the training data and CV data to the observed frequency array
         ...
         y_pcll<-clnmmj(x_tr, g=s, n_level, lambda=Lambda[t], initmax, itmax, tol_logL, init_para = NULL)
         pcllcv[s-min_G+1,t,i] <- loglike_pred.clnmmj(n_level, obs_freqcv, y_pcll$pivec, y_pcll$theta)
        }
...
#      occur1=which.max(cCLC) 
#      loc1=arrayInd(occur1,dim(cCLC)) 
#      opt_para[1,] <- c(G[loc1[1,1]],Lambda[loc1[1,2]],cCLC[loc1[1,1],loc1[1,2]])
